...
public class FindDemo {
	 ...
	 * if the word starts with our letter into the for loop condition. This
	...
	private static void setDemoTraditionalFor() {
		...
		for (String letter : letters) {
			...
			for (String animal = set.ceiling(letter); animal != null
					&& animal.startsWith(letter); animal = set.higher(animal)) {
	...
	private static void streamApproach() {
		...
		for (String letter : letters) {
			...
			Set<String> words = Stream.of(animals)
					// .parallel();
					.filter(w -> w.startsWith(letter)).collect(Collectors.toSet());
